...
export default function OnboardingScreen() {
  ...
  const wrap = (children: React.ReactNode, canNext?: boolean, onNext?: () => void, onBack?: () => void) => (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: 'var(--color-background)', padding: '60px 28px 40px' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginTop: 24 }}>
        <OrangeButton onClick={canNext === false ? undefined : (onNext ?? next)} style={{ opacity: canNext === false ? 0.4 : 1, pointerEvents: canNext === false ? 'none' : 'auto' }}>
          Continue
        </OrangeButton>
        {onBack !== undefined && (
  ...
  if (step === 'welcome') return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: 'var(--color-background)', padding: '60px 28px 40px', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <h1 style={{ fontSize: 32, fontWeight: 700, fontFamily: 'Inter, sans-serif', color: 'var(--color-foreground)', margin: '0 0 12px', textAlign: 'center' }}>MedLedger</h1>
      <p style={{ fontSize: 16, color: 'var(--color-muted-foreground)', fontFamily: 'Inter, sans-serif', textAlign: 'center', lineHeight: 1.6, margin: '0 0 48px' }}>
  ...
  if (step === 'name') return wrap(
    <>
      <h2 style={{ fontSize: 26, fontWeight: 700, fontFamily: 'Inter, sans-serif', color: 'var(--color-foreground)', margin: '0 0 8px' }}>What's your name?</h2>
      <p style={{ fontSize: 15, color: 'var(--color-muted-foreground)', fontFamily: 'Inter, sans-serif', margin: '0 0 28px', lineHeight: 1.5 }}>
  ...
  if (step === 'disclaimer') {
    ...
    return wrap(
      <>
        <h2 style={{ fontSize: 26, fontWeight: 700, fontFamily: 'Inter, sans-serif', color: 'var(--color-foreground)', margin: '0 0 8px' }}>Important disclaimer</h2>
        <p style={{ fontSize: 15, color: 'var(--color-muted-foreground)', fontFamily: 'Inter, sans-serif', margin: '0 0 24px' }}>Please read before continuing.</p>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16, marginBottom: 24 }}>
          {sections.map(s => (
            <div key={s.title}>
              <p style={{ fontSize: 11, fontWeight: 700, color: 'var(--color-primary)', margin: '0 0 4px', fontFamily: 'Inter, sans-serif', letterSpacing: '0.5px' }}>{s.title}</p>
        ...
        <button onClick={() => setDisclaimerChecked(v => !v)} style={{ display: 'flex', alignItems: 'center', gap: 12, background: 'var(--color-card)', border: '1px solid var(--color-border)', borderRadius: 12, padding: '14px 16px', width: '100%', cursor: 'pointer' }}>
          ...
          <span style={{ fontSize: 14, fontFamily: 'Inter, sans-serif', color: 'var(--color-foreground)', textAlign: 'left' }}>I have read and understand this disclaimer</span>
        </button>
      </>,
      disclaimerChecked,
      next,
  ...
  if (step === 'meds') return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: 'var(--color-background)', padding: '60px 28px 40px' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        <h2 style={{ fontSize: 26, fontWeight: 700, fontFamily: 'Inter, sans-serif', color: 'var(--color-foreground)', margin: '0 0 8px' }}>Your medications</h2>
        <p style={{ fontSize: 15, color: 'var(--color-muted-foreground)', fontFamily: 'Inter, sans-serif', margin: '0 0 24px', lineHeight: 1.5 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 16 }}>
            {meds.map((m, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'var(--color-card)', border: '1px solid var(--color-border)', borderRadius: 14, padding: '12px 16px' }}>
                <div>
                  <p style={{ fontSize: 15, fontWeight: 600, fontFamily: 'Inter, sans-serif', color: 'var(--color-foreground)', margin: '0 0 2px' }}>{m.name}</p>
                  <p style={{ fontSize: 12, color: 'var(--color-muted-foreground)', fontFamily: 'Inter, sans-serif', margin: 0 }}>{m.dose}{m.unit} · {m.frequency} · {m.time}</p>
                </div>
        ...
        {addingMed ? (
          <div style={{ background: 'var(--color-card)', border: '1px solid var(--color-border)', borderRadius: 16, padding: 16, marginBottom: 16 }}>
            <div style={{ marginBottom: 12 }}>
              <p style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.5px', textTransform: 'uppercase', color: 'var(--color-muted-foreground)', margin: '0 0 6px', fontFamily: 'Inter, sans-serif' }}>Medication name</p>
              <Input placeholder="e.g. Metformin" value={medForm.name} onChange={v => setMedForm(p => ({ ...p, name: v }))} />
            </div>
            <div style={{ display: 'flex', gap: 10, marginBottom: 12 }}>
              <div style={{ flex: 1 }}>
                <p style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.5px', textTransform: 'uppercase', color: 'var(--color-muted-foreground)', margin: '0 0 6px', fontFamily: 'Inter, sans-serif' }}>Dose</p>
                <Input placeholder="500" value={medForm.dose} onChange={v => setMedForm(p => ({ ...p, dose: v }))} />
              </div>
              <div style={{ width: 110 }}>
                <p style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.5px', textTransform: 'uppercase', color: 'var(--color-muted-foreground)', margin: '0 0 6px', fontFamily: 'Inter, sans-serif' }}>Unit</p>
            ...
            <div style={{ marginBottom: 12 }}>
              <p style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.5px', textTransform: 'uppercase', color: 'var(--color-muted-foreground)', margin: '0 0 6px', fontFamily: 'Inter, sans-serif' }}>Frequency</p>
            ...
            <div style={{ marginBottom: 12 }}>
              <p style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.5px', textTransform: 'uppercase', color: 'var(--color-muted-foreground)', margin: '0 0 6px', fontFamily: 'Inter, sans-serif' }}>Time to take</p>
              <Input placeholder="8:00 AM" value={medForm.time} onChange={v => setMedForm(p => ({ ...p, time: v }))} />
            </div>
            <div style={{ marginBottom: 16 }}>
              <p style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.5px', textTransform: 'uppercase', color: 'var(--color-muted-foreground)', margin: '0 0 6px', fontFamily: 'Inter, sans-serif' }}>Reason (optional)</p>
          ...
          >
            ...
            <span style={{ fontSize: 14, fontFamily: 'Inter, sans-serif', color: 'var(--color-primary)', fontWeight: 600 }}>Add a medication</span>
          </button>
        )}
      </div>
  ...
  if (step === 'prefs') return wrap(
    <>
      <h2 style={{ fontSize: 26, fontWeight: 700, fontFamily: 'Inter, sans-serif', color: 'var(--color-foreground)', margin: '0 0 8px' }}>Personalise</h2>
      <p style={{ fontSize: 15, color: 'var(--color-muted-foreground)', fontFamily: 'Inter, sans-serif', margin: '0 0 28px' }}>Choose your look and feel.</p>

      <p style={{ fontSize: 13, fontWeight: 600, letterSpacing: '0.5px', textTransform: 'uppercase', color: 'var(--color-muted-foreground)', margin: '0 0 12px', fontFamily: 'Inter, sans-serif' }}>Accent colour</p>
      ...
      <div style={{ background: 'var(--color-card)', border: '1px solid var(--color-border)', borderRadius: 14 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px' }}>
          <div>
            <p style={{ fontSize: 15, fontWeight: 600, fontFamily: 'Inter, sans-serif', color: 'var(--color-foreground)', margin: '0 0 2px' }}>Dark mode</p>
            <p style={{ fontSize: 13, color: 'var(--color-muted-foreground)', fontFamily: 'Inter, sans-serif', margin: 0 }}>Easier on the eyes at night</p>
          </div>
  ...
  if (step === 'done') return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: 'var(--color-background)', padding: '60px 28px 40px', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <h1 style={{ fontSize: 28, fontWeight: 700, fontFamily: 'Inter, sans-serif', color: 'var(--color-foreground)', margin: '0 0 12px', textAlign: 'center' }}>
