      ...
      <div className="scrollable" style={{ flex: 1, overflowY: 'auto', padding: '16px 20px' }}>
        ...
        {sorted.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '48px 0' }}>
            <p style={{ fontSize: 15, color: 'var(--color-muted-foreground)', fontFamily: 'Inter, sans-serif' }}>No logs yet. Start logging your medications!</p>
          </div>
        ) : (
          <>
            {/* Photo grid */}
            {withPhotos.length > 0 && (
              <>
                <p style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.5px', textTransform: 'uppercase', color: 'var(--color-muted-foreground)', margin: '0 0 12px', fontFamily: 'Inter, sans-serif' }}>Photos</p>
                  ...
                  {withPhotos.map(log => (
                    <div key={log.id} style={{ borderRadius: 14, overflow: 'hidden', position: 'relative' }}>
                      ...
                      <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: 'linear-gradient(transparent, rgba(0,0,0,0.65))', padding: '20px 10px 8px' }}>
                        <p style={{ margin: 0, fontSize: 12, fontWeight: 600, color: 'white', fontFamily: 'Inter, sans-serif' }}>{getMedName(log.medicationId)}</p>
            ...
            {/* All logs list */}
            <p style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.5px', textTransform: 'uppercase', color: 'var(--color-muted-foreground)', margin: '0 0 12px', fontFamily: 'Inter, sans-serif' }}>All Logs</p>
              ...
              {sorted.map(log => (
                <Card key={log.id} style={{ padding: 14, display: 'flex', alignItems: 'center', gap: 12 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <p style={{ fontSize: 12, color: 'var(--color-muted-foreground)', fontFamily: "'JetBrains Mono', monospace", margin: 0 }}>
                      {formatDate(log.timestamp)}
                    </p>
                  </div>
                  <span style={{ fontSize: 11, fontWeight: 600, color: '#004D1A', background: '#DFE6E1', borderRadius: 999, padding: '3px 8px', flexShrink: 0, fontFamily: 'Inter, sans-serif' }}>
                    Taken
                  </span>
                </Card>
              ))}
